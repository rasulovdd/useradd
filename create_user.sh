...
echo "Версия: 1.3 (Rocky Linux compatible)"
echo -e "\033[0m"
...
# Восстанавливает SELinux-контексты для указанного пути (если SELinux активен)
restorecon_if_selinux() {
    ...
    if command_exists restorecon && command_exists getenforce; then
        ...
        if [ "$mode" = "Enforcing" ] || [ "$mode" = "Permissive" ]; then
            restorecon -Rv "$path" >/dev/null 2>&1
        fi
    fi
}
...
# Определяет алгоритм хеширования паролей, поддерживаемый системой
detect_crypt_method() {
    local method
...
create_user() {
    ...
    # Rocky Linux: явно указываем группу через -g вместо -U, чтобы избежать
    # конфликта с USERGROUPS_ENAB=no в /etc/login.defs некоторых конфигураций
    if ! groupadd -f "$username" 2>/dev/null; then
        log_warning "Не удалось создать группу '$username', будет использована группа users"
    fi

    if useradd -m -s /bin/bash -g "$username" "$username" 2>/dev/null || \
    ...
    # Rocky Linux: chpasswd поддерживает SHA-512 по умолчанию
    ...
    # Rocky Linux: восстанавливаем SELinux-контекст для домашней директории
    restorecon_if_selinux "$HOME_DIR"
    ...
    if [ -n "$ssh_key" ]; then
        ...
        # Rocky Linux: критично для SSH — восстанавливаем SELinux-контекст
        restorecon_if_selinux "$HOME_DIR/.ssh"
        ssh_key_added=1
    ...
    if [ -z "$sudo_choice" ] || [[ "$sudo_choice" =~ ^[yY] ]]; then
        ...
        if [ -z "$admin_group" ]; then
            ...
            if usermod -aG "$admin_group" "$username"; then
                ...
                # Rocky Linux: гарантируем, что wheel разрешён в sudoers
                if [ "$admin_group" = "wheel" ]; then
                    if ! grep -qE '^%wheel\s+ALL=\(ALL\)' /etc/sudoers 2>/dev/null; then
                        if [ -d /etc/sudoers.d ]; then
                            echo "%wheel ALL=(ALL) ALL" > /etc/sudoers.d/wheel
                            chmod 440 /etc/sudoers.d/wheel
                            log_info "Добавлен файл /etc/sudoers.d/wheel для группы wheel"
                        fi
                    fi
                fi
            else
...
delete_user() {
    ...
    if userdel "$username"; then
        ...
        # Удаляем приватную группу пользователя, если она осталась
        if getent group "$username" >/dev/null 2>&1; then
            groupdel "$username" 2>/dev/null && log_info "Группа '$username' удалена"
        fi
    else
